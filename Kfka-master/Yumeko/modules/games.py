...
from Yumeko.database.game_db import *
import asyncio
import json
...
async def _bal(client, message):
    user = message.from_user
    if not await is_player(user.id):
        await create_account(user.id, user.username or "NoUsername")
...
@app.on_message(filters.command("setdalcs", prefixes=config.COMMAND_PREFIXES))
async def _bal(client,message):
    user = message.from_user
...
__help__ = """
🎲 **𝙂ᴀᴍᴇ 𝙲𝚘𝚖𝚖𝚊𝚗𝚍𝚜 & ᴅᴀʟᴄs ᴇᴄᴏɴᴏᴍʏ**

Manage your dalcs, play fun betting games, and climb the leaderboard!

💰 **Economy Commands:**
- `/daily` — Claim your daily bonus of 10,000 dalcs.
- `/weekly` — Claim your weekly bonus of 50,000 dalcs.
- `/bal` or `/balance` or `/dalcs` — Check your current dalcs balance.
- `/pay <amount>` (reply) — Pay dalcs to another user by replying to their message.
- `/setdalcs` (reply, SUDO only) — Set dalcs for a user manually.

🎮 **Game Commands:**
- `/trivia` — Test your knowledge! Answer trivia questions to win dalcs.
- `/bet <amount> <heads/tails>` — Bet on a coin toss.
- `/dart <amount>` — Play a dart dice game.
- `/bowl <amount>` — Play a bowling dice game.
- `/basket <amount>` — Play a basketball dice game.

🏆 **Leaderboard:**
- `/gametop` or `/leaderboard` — View the global dalcs leaderboard.

⏳ Cooldowns apply to games and bonuses. Have fun and play smart!

*Note:* Amounts can be `*` to use your full balance in bets/payments.
"""