...
async def afk_command(client: Client, message: Message):
    ...
    if afk_reason:
        afk_message += f"\n**Reason:** {afk_reason}"
    x = await message.reply(afk_message)
    await sleep(180)
    await x.delete()


@app.on_message(filters.all & ~filters.me & ~filters.command(["afk" , "brb"] , prefixes=config.COMMAND_PREFIXES) & ~filters.regex(r"(?i)^off(\s+.+)?$")  & ~filters.regex(r"(?i)^brb(\s+.+)?$"), group=AFK_REPLY_GROUP)
@error
@save
async def afk_mention_handler(client: Client, message: Message):
    ...
    if afk_data.get("afk_reason") and afk_data.get("media_id"):
        ...
        x = await message.reply(f"{afk_data['user_first_name']} {random_msg}.\n**𝖠𝖥𝖪 𝖿𝗈𝗋** : {elapsed_str}")
    await sleep(180)
    if x:
        await x.delete()
    else:
