...
async def announce_member_update(client: Client, chat_member_updated: ChatMemberUpdated):
    ...
    if old_status in [ChatMemberStatus.MEMBER, ChatMemberStatus.RESTRICTED] and new_status in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.OWNER]:
        if new_title:
            message_text = (
                f"{target_user_mention} is promoted by {from_user_mention} with the title {new_title}"
            )
        else:
            message_text = f"{target_user_mention} is promoted by {from_user_mention}."
    ...
    # Handle Demotion
        ...
        if new_status == ChatMemberStatus.MEMBER:
            message_text = f"{target_user_mention} is demoted by {from_user_mention}."
        else:
            message_text = f"{target_user_mention} is demoted and banned by {from_user_mention}."
        log_action = "Demotion"

    # Handle Mute/Unmute
    elif old_status == ChatMemberStatus.RESTRICTED and new_status == ChatMemberStatus.MEMBER:
        message_text = f"{target_user_mention} is unmuted by {from_user_mention}."
        log_action = "Unmute"

    elif new_status == ChatMemberStatus.RESTRICTED and chat_member_updated.new_chat_member.is_member:
        message_text = f"{target_user_mention} is muted by {from_user_mention}."
        log_action = "Mute"
    ...
    # Handle Title Change (for admins only)
        ...
        if old_title != new_title:
            if new_title is None:
                message_text = (
                    f"{target_user_mention}'s title is removed by {from_user_mention}.\n"
                    f"Old title: <code>{old_title}</code>\nNew title: None\n\n"
                )
            elif old_title is None:
                message_text = (
                    f"{target_user_mention}'s title is set by {from_user_mention}.\n"
                    f"New title: {new_title}</pre>"
                )
            else:
                message_text = (
                    f"{target_user_mention}'s title is changed by {from_user_mention}.\n"
                    f"Old title: <code>{old_title}</code>\nNew title: {new_title}"
                )

    # Handle Banning/Unbanning
    elif new_status == ChatMemberStatus.BANNED:
        message_text = f"{target_user_mention} is banned by {from_user_mention}."
        log_action = "Ban"

    elif old_status == ChatMemberStatus.BANNED and new_status != ChatMemberStatus.BANNED:
        message_text = f"{target_user_mention} is unbanned by {from_user_mention}."
        log_action = "Unban"
