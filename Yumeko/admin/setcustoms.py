...
async def set_custom_title(client: Client, message: Message):
    if not message.reply_to_message and len(message.command) < 3:
        await message.reply_text(
            "Usage: /settag (username/user_id) (custom_tag) or reply to a user's message with /settag (custom_tag)"
        )
        return
...
async def set_custom_bio(client: Client, message: Message):
    if not message.reply_to_message and len(message.command) < 3:
        await message.reply_text(
            "Usage: /setbio (username/user_id) (custom_bio) or reply to a user's message with /setbio (custom_bio)"
        )
        return
